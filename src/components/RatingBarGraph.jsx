const FullStar = ({ color }) => (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill={"#FFD700"} className="w-[20px] h-full">
...
const RatingBarGraph = ({ distribution }) => {
    ...
    return (
      <div className="w-full grid grid-cols-11 items-end gap-1 min-h-[50px]">
        {/* Left Label */}
        <div className=" flex w-[100%] text-right gap-1 items-center">
            <h1>1</h1>
            <FullStar />
        </div>
        ...
        {distribution.map((count, index) => {
          ...
          return (
            <div
              key={index}
              className="bg-gray-300 hover:bg-primaryPurple-500 transition-all duration-300 rounded-t"
